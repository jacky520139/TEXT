...
code unsigned char  Reg[][2] = {
    ...
    {STEP_DELTA,   Bin(00000000)},		//计步差值控制
    {STEP_WTM0,    Bin(00000000)},		//计步阈值低位设置寄存器
    {STEP_WTM,     Bin(00000000)},		//计步阈值高位设置寄存器
    {STEP_CONFIG1, Bin(00000000)},		//计步值控制寄存器1
    {STEP_CONFIG2, Bin(00000000)},		//计步值控制寄存器2
    {CTRL_REG0,    Bin(00000000)},		//计步控制器，b6:温度传感器，B4:滤波数据,B3:高性能,B2:低通滤波，B1~B0:滤波频率
    {CTRL_REG1,    Bin(01000111)},		//B7~B4:数据率，B3:0正常，1低功耗，B2:Z轴使能，B2:Y轴使能，B2:X轴使能，
    {CTRL_REG2,    Bin(00000000)},		//B7~B6:高通模式选择,B5~B4:高通截止频率选择,B3:数据滤波选择,B2:CLICK 功能高通滤波使能,B1中断 2 AOI 功能高通滤波使能,B0:中断 1 AOI 功能高通滤波使能
