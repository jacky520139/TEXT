...
switch(PIF)
{
	case   1:
				 Lignt_VDD=1;
		     adc_init();
         Light_Val=ADC_Read(7)>>4;//读取光强度
	       Light_Val=0xff-Light_Val;
	       ADCCON1&=~0x01;//关闭ADC电源
	       Lignt_VDD=0;
         LED_OPEN_Time=(Default_LED_OPEN_Time*Light_Val)>>8;
	       LED_Duty=((LED_cycle*Light_Val)>>8)+1; 
         LED_OPEN_T=(0xff-LED_OPEN_Time);
         LED_CLOSE_T=(0xff-(126-LED_OPEN_Time));
         TIME_S=0;
	       TIME_MIN=0;
//	       clr_EPI;//关闭管脚中断
		     set_WKTR; //开启唤醒定时器 
...
//	Scan_LED1();
	 Light_Val=ADC_Read(7)>>4;//读取光强度
		 Light_Val=0xff-Light_Val;
		if(Light_Val>OLD_Light_Val)
		Light_T=Light_Val-OLD_Light_Val;
...
//		    
////     LED_OPEN_Time=(Default_LED_OPEN_Time*Light_Val)>>8;
////	   LED_Duty=(LED_cycle*Light_Val)>>8; 
////		
////     LED_OPEN_T=(0xff-LED_OPEN_Time);
////     LED_CLOSE_T=(0xff-(126-LED_OPEN_Time));
//			ANO_DT_Send_Version(0XF1,Light_Val, OLD_Light_Val,Light_T , 0, 0);
//      OLD_Light_Val=Light_Val;
