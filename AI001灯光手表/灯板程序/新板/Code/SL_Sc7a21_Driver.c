...
void SL_SC7A21_Pedo_Config(unsigned char *pedo_reg_value)
{
    ...
    //INT_SET  STEP_EN  0x05,0xA4,0x1F,0x37,   0x0F,0xFF,0x00
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_CTRL_REG0, pedo_reg_value[0]);//计步控制寄存器 0
    //STEP_SET
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_CTRL_REG1, pedo_reg_value[1]);//计步值控制寄存器 1
    //STEP_SET
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_CTRL_REG2, pedo_reg_value[2]);//计步值控制寄存器 2
    //STEP_SET
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_CTRL_REG3, pedo_reg_value[3]);//计步值控制寄存器 3
    //DELTA
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_DELTA_REG, pedo_reg_value[4]);//计步差值控制寄存器
    //WTM_SET
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_WTM_REG0, pedo_reg_value[5]);//WTM0_SET计步阈值设置寄存器 低位
    SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_STEP_WTM_REG1, pedo_reg_value[6]);//WTM1_SET计步阈值设置寄存器 高位
...
unsigned int  SL_SC7A21_Get_Pedo_Count(void)
{
    ...
    if(SL_Step_Value-SL_Step_Value_Temp>100)//如果1秒钟步数超过100步，则为异常，步更新步数
    {
...
{
	//if config the click function 0x01,0x3C,0x82,0x15,0x48,0x04,0x17
	...
	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_CTRL_REG2, click_para[1]);//使能高通滤波器High-Pass 0x3C
	
	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_CTRL_REG6, click_para[2]);//CLICK_INT2 中断使能 0x82
	
	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_CLICK_CFG, click_para[3]);//xyz轴单击中断使能控制 0x15

	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_CLICK_THS_REG, click_para[4]);//单双击触发的阈值 0x48
	
	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_TIME_LIMIT_REG, click_para[5]);//单双击检测时间的阈值 0x04
	
	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_TIME_LATENCY_REG, click_para[6]);//延迟时间 0x17
	
	SL_SC7A21_I2c_Spi_Write(SL_SC7A21_SPI_IIC_MODE,SL_SC7A21_CTRL_REG4, 0x90);//Disable DLPF 4g Scale禁止低通滤波 4g

}

unsigned int SL_Click_Sqrt(unsigned int sqrt_data)//平方根
{
...
unsigned char SL_SC7A21_Click_Algo(unsigned int Click_TH,unsigned char Click_E)//Service function in Int Function
{
	...
	for(i=0;i<fifo_len;i++)
	{
		...
		sc7a20_data= SL_Click_Sqrt(sc7a20_data);//平方根
