...
signed char SL_SC7A21_INIT(void)
{
    ...
    unsigned char pedo_para[7] ={0x05,0xA4,0x1F,0x37,0x0F,0xFF,0x00};//计步，抬手，睡眠
    unsigned char wrist_para[4]={0x31,0x35,0x00,0x93};//中断配置，抬手参数配置（08~0B）
...
/*****if use spi mode please config first******/    
    ...
    status=SL_SC7A21_Power_Config(SL_SC7A21_ODR_50HZ,SL_SC7A21_LOWER_POWER);//ODR配置，工作模式配置
