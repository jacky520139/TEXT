...
unsigned char SL_SC7A21_I2c_Spi_Write(unsigned char sl_spi_iic,unsigned char reg, unsigned char Data)
{   if(sl_spi_iic)
    i2cs_tx_data(SC7A21_IIC_ADDRESS,reg,&Data,1);
    return 1;

}
unsigned char SL_SC7A21_I2c_Spi_Read(unsigned char sl_spi_iic,unsigned char reg, unsigned char len, unsigned char *buf)
{
   if(sl_spi_iic)
	  i2cs_rx_data(SC7A21_IIC_ADDRESS,reg,buf,len);
    return 1;
}
